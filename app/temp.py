...
coll = db["vehicle_inventory"]
all_plates = [p for p in coll.distinct("LicensePlateNumber") if p]
result = [p for p in all_plates if p not in keep]
print(f"Deleting {len(result)} documents; keeping {sorted(keep)}")

db['vehicle_inventory'].delete_many({'LicensePlateNumber': {"$in": result}})
